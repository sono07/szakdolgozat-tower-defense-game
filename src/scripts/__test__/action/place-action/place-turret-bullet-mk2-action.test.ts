...
import { PlaceTurretBulletMk2Action } from "../../../impl/action/place-action/place-turret-bullet-mk2-action.class";
import { PlaceAction } from "../../../impl/action/place-action/__abstract__/place-action.abstract";
import { ACTION_KEY_PLACE_TURRET_BULLET_MK2 } from "../../../impl/game-constants";
...
test('Test place turret bullet mk2 construct', () => {
    placeActionMockHelper.setup((proto) => { })

    const group = jest.fn();
    const gameStateStore = jest.fn<IGameStateStore, any[]>((...args: any[]) => ({
        turretBulletMk2sGroup: group
    } as any))();

    const action = new PlaceTurretBulletMk2Action(gameStateStore);
...
test('Test place turret bullet mk2 action key', () => {
    placeActionMockHelper.setup((proto) => { })
    ...
    expect(action.actionKey).toStrictEqual(ACTION_KEY_PLACE_TURRET_BULLET_MK2);  
})
