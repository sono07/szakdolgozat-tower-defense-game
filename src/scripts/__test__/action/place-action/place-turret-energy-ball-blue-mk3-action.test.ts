...
import { PlaceTurretEnergyBallBlueMk3Action } from "../../../impl/action/place-action/place-turret-energy-ball-blue-mk3-action.class";
import { PlaceAction } from "../../../impl/action/place-action/__abstract__/place-action.abstract";
import { ACTION_KEY_PLACE_TURRET_ENERGY_BALL_BLUE_MK3 } from "../../../impl/game-constants";
...
test('Test place turret energy ball blue mk3 construct', () => {
    placeActionMockHelper.setup((proto) => { })

    const group = jest.fn();
    const gameStateStore = jest.fn<IGameStateStore, any[]>((...args: any[]) => ({
        turretEnergyBallBlueMk3sGroup: group
    } as any))();

    const action = new PlaceTurretEnergyBallBlueMk3Action(gameStateStore);
...
test('Test place turret energy ball blue mk3 action key', () => {
    placeActionMockHelper.setup((proto) => { })
    ...
    expect(action.actionKey).toStrictEqual(ACTION_KEY_PLACE_TURRET_ENERGY_BALL_BLUE_MK3);  
})
