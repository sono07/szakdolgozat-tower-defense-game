...
import { PlaceTurretEnergyBallOrangeMk1Action } from "../../../impl/action/place-action/place-turret-energy-ball-orange-mk1-action.class";
import { PlaceAction } from "../../../impl/action/place-action/__abstract__/place-action.abstract";
import { ACTION_KEY_PLACE_TURRET_ENERGY_BALL_ORANGE_MK1 } from "../../../impl/game-constants";
...
test('Test place turret energy ball orange mk1 construct', () => {
    placeActionMockHelper.setup((proto) => { })

    const group = jest.fn();
    const gameStateStore = jest.fn<IGameStateStore, any[]>((...args: any[]) => ({
        turretEnergyBallOrangeMk1sGroup: group
    } as any))();

    const action = new PlaceTurretEnergyBallOrangeMk1Action(gameStateStore);
...
test('Test place turret energy ball orange mk1 action key', () => {
    placeActionMockHelper.setup((proto) => { })
    ...
    expect(action.actionKey).toStrictEqual(ACTION_KEY_PLACE_TURRET_ENERGY_BALL_ORANGE_MK1);  
})
