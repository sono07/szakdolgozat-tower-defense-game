...
import { PlaceTurretLaserMk1Action } from "../../../impl/action/place-action/place-turret-laser-mk1-action.class";
import { PlaceAction } from "../../../impl/action/place-action/__abstract__/place-action.abstract";
import { ACTION_KEY_PLACE_TURRET_LASER_MK1 } from "../../../impl/game-constants";
...
test('Test place turret laser mk1 construct', () => {
    placeActionMockHelper.setup((proto) => { })

    const group = jest.fn();
    const gameStateStore = jest.fn<IGameStateStore, any[]>((...args: any[]) => ({
        turretLaserMk1sGroup: group
    } as any))();

    const action = new PlaceTurretLaserMk1Action(gameStateStore);
...
test('Test place turret laser mk1 action key', () => {
    placeActionMockHelper.setup((proto) => { })
    ...
    expect(action.actionKey).toStrictEqual(ACTION_KEY_PLACE_TURRET_LASER_MK1);  
})
