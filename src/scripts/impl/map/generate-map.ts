...
function createGraph(tiles: number[][], p1: number[], p2: number[]) {
    ...
    for (let i = from[0]; i < to[0] + 1; i++) {
        for (let j = from[1]; j < to[1] + 1; j++) {
            const name = pointToPathString([i, j]);
            const neighbours: Map<number> = {};

            if (i > from[0]) neighbours[pointToPathString([i-1, j])] = tiles[i-1][j] - tiles[i][j] + 5;
            if (i < to[0])   neighbours[pointToPathString([i+1, j])] = tiles[i+1][j] - tiles[i][j] + 5;
            if (j > from[1]) neighbours[pointToPathString([i, j-1])] = tiles[i][j-1] - tiles[i][j] + 5;
            if (j < to[1])   neighbours[pointToPathString([i, j+1])] = tiles[i][j+1] - tiles[i][j] + 5;
...
function removeNodesFromGraph(graph: Graph, nodesToRemove: string[]) {
    for (let j = 0; j < nodesToRemove.length - 1; j++) {
        graph.removeNode(nodesToRemove[j]);
    }
}

function pointToPathString(point: Tuple<number, 2>) {
    return `${point[0]}_${point[1]}`;
}

function pathStringToPoint(str: String): Tuple<number, 2> {
...
function differenceToSide(value: Tuple<number, 2>) {
    if(value[0] == 1) {
        return SIDE_RIGHT;
    } else if(value[0] == -1) {
        return SIDE_LEFT;
...
export function generateMap(seed: string, gridHeight: number, gridWidth: number): {map: number[][], path: Tuple<number,2>[]} {
    ...
    for (let i = 0; i < fixPoints.length - 1; i++) {
        ...
        const partialPath: string[] = graph.path(
            pointToPathString(startPoint),
            pointToPathString(endPoint),
        );
    ...
    const pointPath: Tuple<number, 2>[] = [
        [0, -1],
        ...path.map(p => pathStringToPoint(p)),
    ...
    for (let i = 1; i < pointPath.length - 1; i++) {
        ...
        const d1 = differenceToSide([currentPoint[0] - previousPoint[0], currentPoint[1] - previousPoint[1]]);
        const d2 = differenceToSide([nextPoint[0] - currentPoint[0], nextPoint[1] - currentPoint[1]]);
    ...
    return {
        map: map,
        path: path.map(p => pathStringToPoint(p)),
    }
}
