...
export class TurretBulletMk3Object extends BaseObject {
    ...
    private addProjectile(enemy: IEnemy, angle: number) {
        ...
        const bullet1 = this.objectStore.bulletsGroup.get();
        if (bullet1) {
        ...
        const bullet2 = this.objectStore.bulletsGroup.get();
        if (bullet2) {
        ...
        const bullet3 = this.objectStore.bulletsGroup.get();
        if (bullet3) {
            const fromPos = this.position.clone().add(forwardOffset.clone().add(sideOffset.clone().scale(-1)));
            const targetPos = this.position.clone().add(dirRadius).add(sideOffset.clone().scale(-1));
            bullet3.init(
    ...
    update(time: number, delta: number): void {
        if (time > this.canShootAfterTimeMs) {
            if(this.fire()) {
                this.canShootAfterTimeMs = time + 400;
            }
        }
    }
